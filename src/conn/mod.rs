...
impl Conn {
    ...
    /// This function handles IO error on a wrapped stream.
    pub(crate) fn handle_broken(&mut self) {
    ...
    pub(crate) async fn read_packet(&mut self) -> Result<Vec<u8>> {
        let packet = crate::io::ReadPacket::new(&mut *self).await?;
    ...
    where
        T: Into<Vec<u8>>,
    {
        Ok(crate::io::WritePacket::new(&mut *self, data.into()).await?)
    }
...
mod test {
    ...
    use crate::{
        ...
        Error, OptsBuilder, TxOpts, WhiteListFsLocalInfileHandler,
    };
